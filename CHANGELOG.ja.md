...
- モデルの選択肢を **GPT-5.6 Luna**（初期値）と **Codexの既定モデル** に整理。「Codexの既定モデル」はCodex側の設定に従います。以前の対象外の設定値も同じ動作となり、既存の明示的なユーザー設定は維持します。
- モデル選択を全ワークスペース共通のユーザー設定に変更し、ワークスペースからの上書きを無効化。
- 設定・選択肢の説明とコマンド名を、日本語・韓国語のUI言語に対応。

## 0.5.0

- モデル選択と既定モデルへの再試行、要約／詳細モード、日英韓のプロンプト管理を追加。
- 旧プロンプト設定をカスタムプロファイルへ移行し、保存後に旧設定を削除。
- モデル切替時に古い接続が復活する競合を防止。
